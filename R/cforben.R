...
#' @param nresample The c-for-benefit uses matching and in case of unequal
#' numbers in the treatment groups, the c-for-benefit is based on repeated
#' matching of the smallest group to equal sized subsamples of the larger
#' group. nresample denotes the number of replications of this procedure.
#' @param get.all get the c-for-ben for all resamplings.
#'
#' @return The c-for-benefit estimate (as averaged over the number of
#' replicatations specified in nresample).
#' @export
#'
#' @examples
#' # generate some  data
...
#' ind.A <- which(trt == 0)
#' ind.B <- which(trt == 1)
#' cforben(deltahat, y, ind.A, ind.B, nresample = 100)
#'
#' hist(cforben(deltahat, y, ind.A, ind.B, nresample = 250, get.all=TRUE))
#' abline(v=cforben(deltahat, y, ind.A, ind.B, nresample = 100), col="red")

cforben <- function(deltahat, y, ind.A, ind.B, nresample=1, get.all=FALSE){
